...
echo "============================================"
echo "  Financial Fraud Agent Harness Workshop — Runtime Setup (one-time)"
echo "============================================"
...
# the FINANCE seed will already have partially failed against slim, leaving
...
if [ -n "$EXISTING_IMAGE" ] && echo "$EXISTING_IMAGE" | grep -q 'slim'; then
  ...
  docker volume rm financial-fraud-agent-harness-workshop_oracle-data 2>/dev/null || true
  docker volume rm financial-fraud-agent-harness-workshop-lightweight_oracle-data 2>/dev/null || true
...
# --- 5. Run app/scripts/seed.py — FINANCE + duality views + skill ingestion + scan ---
echo ""
echo "[5/6] Running app/scripts/seed.py (FINANCE + duality views + oracle/skills)..."
