...
5. **Call `remember` for corrections.** The user telling you "amount_cents is in cents" is institutional knowledge — persist it.
...
After Turn 3, query the OAMP store and you'll see a new memory with `metadata.kind = "correction"`. From now on, asking about transaction amounts triggers `search_knowledge` and the correction surfaces — the agent has *learned*.
