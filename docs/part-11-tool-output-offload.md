...
A 50-row `run_sql` of `transactions` is ~10 KB. A `skillbox` body for `agent/ora-error-catalog` is ~25 KB. Three of those in one turn and you're at 100 KB of context for outputs the model only skimmed once.
...
1. Turn N: model calls `run_sql("SELECT * FROM transactions")`. Output is 12 KB.
